...
public class Delay30 extends Block
{
    private String texturePath = Main.modid;  
    private int thisBlockID;
    int fuse;
    
    public Delay30 (int id, Material material, String textureName, int parfuse) {
...
*/
    public void primeTnt(World par1World, int par2, int par3, int par4, int par5)
    {
        if (!par1World.isRemote)
        {
            if ((par5 & 1) == 1)
            {
                DelayPrime30 entitytntprimed = new DelayPrime30(par1World, (double)((float)par2 + 0.5F), (double)((float)par3), (double)((float)par4 + 0.5));
