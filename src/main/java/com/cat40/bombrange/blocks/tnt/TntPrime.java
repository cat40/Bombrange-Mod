package com.cat40.bombrange.blocks.tnt;
...
public class TntPrime extends Entity
{
    ...
    public TntPrime(World par1World)
    {
        super(par1World);
        ...
        for(int i=0; i<rings; i++)
        {
        	...
        	{
        		arrowVectors[i][i2] = arrowSpacing*(i2+1);//Math.toRadians((arrowSpacing*(i2+1)));
        	}
        }
    ...
    public TntPrime(World par1World, double par2, double par4, double par6)//EntityLivingBase par8EntityLivingBase
    {
        this(par1World);
        ...
        if (this.fuse-- <= 0)
        {
            ...
            if (!this.worldObj.isRemote)
            {
            	...
            	for(i=0; i<this.rings; i++)
                {
            		...
            		{
	            		EntityTNTPrimed entitylargefireball = new EntityTNTPrimed(this.worldObj, this.posX, this.posY, this.posZ, tntPlacedBy);
	            		...
	            		double y = -3;
	            		entitylargefireball.motionX = x;
	            		entitylargefireball.motionZ = z;
	            		entitylargefireball.motionY = y;
	                    entitylargefireball.posX = this.posX;
	                    entitylargefireball.posY = this.posY + (double)(this.height / 2.0F) + 0.5D;
	                    entitylargefireball.posZ = this.posZ;
	                    /*if(typeFlag)
	                    {
	                    	entitylargefireball.setPotionDamage(4+32+64); //type + magnitue+duration
	                    }
	                    else
	                    {
	                    	entitylargefireball.setPotionDamage(12+32);
	                    }
	                    typeFlag = !typeFlag; */
