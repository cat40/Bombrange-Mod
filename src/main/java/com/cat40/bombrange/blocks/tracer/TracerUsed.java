...
import com.cat40.bombrange.particles.ParticleTracer;
...
public class TracerUsed extends Block
{
    private String texturePath = Main.modid;  
    private int thisBlockID;
    public int particleNum = 25;
    
    public TracerUsed (int id, Material material, String textureName) {
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random p_149734_5_)
    {
    	...
    	{
    		random = Math.random()-0.5F;///2.0F; //random number from -.5 to .5
    		random2 = Math.random()-0.5F;
    		//world.spawnParticle("explode", x+0.5D, y+1.0D, z+0.5D, 0D, 0D, 0D);
    		Minecraft.getMinecraft().effectRenderer.addEffect(new ParticleTracer(world, x+0.5D+random, y+1D, z+0.5D+random2));
    	}
...
*/
    public void primeTnt(World par1World, int par2, int par3, int par4, int par5)
    {
        if (!par1World.isRemote)
        {
            if ((par5 & 1) == 1)
            {
                TracerPrime entitytntprimed = new TracerPrime(par1World, (double)((float)par2 + 0.5F), (double)((float)par3 + 0.5F), (double)((float)par4 + 0.5));
